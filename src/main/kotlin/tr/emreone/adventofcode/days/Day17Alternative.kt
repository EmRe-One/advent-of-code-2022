...
fun Tower.simulate(rockCount: Int, multiply: Long = 1L): Tower {
    ...
    for (rockIndex in 0 until rockCount) {
        ...
        rock.forEachIndexed { y, line ->
            line.forEachIndexed { x, c ->
                stack[starty+y][x+3] = c
            }
        }

        val cols = MutableList<Int>(7){stack.size-1}
        for (x in 1..7)
            for (y in 0 until stack.size)
                if (stack[y][x] == '#') {
                    cols[x-1] = y
                    break
                }
...
fun main() {
    ...
    val indices = Tower(mostCommon=mostCommon)
        .simulate(input.size)
        .indicesOfMostCommon

    val repeatsAfter = indices
        .flatMap { a -> indices.map { Math.abs(a - it) } }
        .filter { it != 0 }
        .groupingBy { it }
        .eachCount()
        .maxBy { it.value }
    ...
    val goal = Tower()
        .simulate(1000)
        .simulate(repeatsAfter)
        .towerHeightLastSim

    for (initial in input.indices step 5) {
